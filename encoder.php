...
function encode($in = './input/', $out= './output/', $word_list = []){
    $input = realpath($in).DIRECTORY_SEPARATOR;
    $output = realpath($out).DIRECTORY_SEPARATOR;

    $algos = hash_algos();
    if(($path = realpath($in)) && is_dir($path)){
	$path .= DIRECTORY_SEPARATOR;
        foreach(scandir($path) as $key => $filename){
            if(in_array($filename, [".", ".."])) continue;
            if(is_dir($path.$filename)) encode($path.$filename, $out, $word_list);
            if(file_exists($path.$filename) && pathinfo($path.$filename, PATHINFO_EXTENSION) == 'php'){
                ...
                while($continue){
                    ...
                    $key = count($word_list) < 1 ? '' : "<?php /* $word */ ";
                    ...
                    $b = 'function '.$funname.'($a,$b){$c=array('.(strlen($key)+strlen($a_)+strlen($c_)+strlen($d_)+strlen($rand1)).','.strlen($c).',32,266);if($b=='.$angka1.'){$d=substr($a,$c[0]+$c[1],$c[2]);}elseif($b=='.$angka2.'){$d=substr($a,$c[0],$c[1]);}elseif($b=='.$angka3.'){$d=trim(substr($a,$c[0]+$c[1]+$c[2]));}return$d;}';
                    $b_ = "eval(base64_decode('".base64_encode($b)."'));";

                    $b = 'function '.$funname.'($a,$b){$c=array('.(strlen($key)+strlen($a_)+strlen($b_)+strlen($c_)+strlen($d_)+strlen($rand1)).','.strlen($c).',32,266);if($b=='.$angka1.'){$d=substr($a,$c[0]+$c[1],$c[2]);}elseif($b=='.$angka2.'){$d=substr($a,$c[0],$c[1]);}elseif($b=='.$angka3.'){$d=trim(substr($a,$c[0]+$c[1]+$c[2]));}return$d;}';
                    $b_ = "eval(base64_decode('".base64_encode($b)."'));";
                    
                    $hash = $hash1 = hash($algo, $key.$a_.$b_.$c_.preg_replace('/__halt_compiler.*/', '', $d_));

                    $b = 'function '.$funname.'($a,$b){$c=array('.(strlen($key)+strlen($a_)+strlen($b_)+strlen($c_)+strlen($d_)+strlen($rand1)).','.strlen($c).','.strlen($hash).',266);if($b=='.$angka1.'){$d=substr($a,$c[0]+$c[1],$c[2]);}elseif($b=='.$angka2.'){$d=substr($a,$c[0],$c[1]);}elseif($b=='.$angka3.'){$d=trim(substr($a,$c[0]+$c[1]+$c[2]));}return$d;}';
                    $b_ = "eval(base64_decode('".base64_encode($b)."'));";
